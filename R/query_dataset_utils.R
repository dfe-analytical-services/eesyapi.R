...
todf_geographies <- function(geographies) {
  ...
  if (any(geog_level_lookup$human_friendly %in% geographies$geographic_level)) {
    geographies <- geographies |>
      dplyr::left_join(geog_level_lookup, by = c("geographic_level" = "human_friendly")) |>
      dplyr::mutate(
  ...
  if (any(geog_level_lookup$human_friendly %in% geographies$location_level)) {
    geographies <- geographies |>
      dplyr::left_join(geog_level_lookup, by = c("location_level" = "human_friendly")) |>
      dplyr::mutate(
        location_level = dplyr::case_when(
          !is.na(api_friendly) ~ api_friendly,
          .default = !!rlang::sym("location_level")
